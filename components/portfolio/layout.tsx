    ...
    <div className="min-h-screen md:h-screen w-full max-w-full transition-colors duration-300 bg-transparent overflow-x-hidden">
      <div className="h-full w-full max-w-full relative p-1 sm:p-2 md:p-6 lg:p-8 box-border">
        ...
        <div className="flex flex-col gap-2 w-full md:grid md:grid-cols-2 md:grid-rows-2 md:gap-2 md:h-full">
          <div className="w-full max-w-full overflow-hidden rounded-2xl order-2 md:order-1 md:h-full"><AboutMeCard isDark={isDark} /></div>
          <div className="w-full max-w-full overflow-hidden rounded-2xl order-1 md:order-2 md:h-full"><WorkExperienceCard isDark={isDark} /></div>
          <div className="w-full max-w-full overflow-hidden rounded-2xl order-3 md:order-3 md:h-full"><SkillsCard isDark={isDark} /></div>
          <div className="w-full max-w-full overflow-hidden rounded-2xl order-4 md:order-4 md:h-full"><ProjectsCard isDark={isDark} /></div>
        </div>
